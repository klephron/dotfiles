export FNOX_SHELL=zsh

fnox() {
  local command
  command="${1:-}"
  if [ "$#" = 0 ]; then
    /usr/bin/fnox
    return
  fi
  shift

  case "$command" in
  deactivate|shell)
    eval "$(/usr/bin/fnox "$command" "$@")"
    ;;
  *)
    /usr/bin/fnox "$command" "$@"
    ;;
  esac
}

_fnox_hook() {
  trap -- '' SIGINT
  eval "$(/usr/bin/fnox hook-env -s zsh)"
  trap - SIGINT
}

typeset -ag precmd_functions
if [[ -z "${precmd_functions[(r)_fnox_hook]+1}" ]]; then
  precmd_functions=( _fnox_hook ${precmd_functions[@]} )
fi

typeset -ag chpwd_functions
if [[ -z "${chpwd_functions[(r)_fnox_hook]+1}" ]]; then
  chpwd_functions=( _fnox_hook ${chpwd_functions[@]} )
fi
#compdef fnox
# @generated by usage-cli from usage spec
local curcontext="$curcontext"

# caching config
_usage_fnox_cache_policy() {
  if [[ -z "${lifetime}" ]]; then
    lifetime=$((60*60*4)) # 4 hours
  fi
  local -a oldp
  oldp=( "$1"(Nms+${lifetime}) )
  (( $#oldp ))
}

_fnox() {
  typeset -A opt_args
  local curcontext="$curcontext" cache_policy

  if ! type -p usage &> /dev/null; then
      echo >&2
      echo "Error: usage CLI not found. This is required for completions to work in fnox." >&2
      echo "See https://usage.jdx.dev for more information." >&2
      return 1
  fi

  local spec_file="${TMPDIR:-/tmp}/usage__usage_spec_fnox_1_25_1.spec"
  if [[ ! -f "$spec_file" ]]; then
    fnox usage >| "$spec_file"
  fi
  local -a completions=()
  while IFS= read -r line; do
    completions+=("$line")
  done < <(command usage complete-word --shell zsh -f "$spec_file" -- "${words[@]}")
  _describe 'completions' completions -S ''
  return 0
}

if [ "$funcstack[1]" = "_fnox" ]; then
    _fnox "$@"
else
    compdef _fnox fnox
fi

# vim: noet ci pi sts=0 sw=4 ts=4
